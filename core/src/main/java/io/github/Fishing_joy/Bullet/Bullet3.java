...
public class Bullet3 extends Bullet {
    ...
    public Bullet3(float x, float y, float angleDeg) {
        this(x, y, angleDeg, DEFAULT_SPEED);
    }

    public Bullet3(float x, float y, float angleDeg, float speed) {
        super(x, y, angleDeg, speed, region, webRegion);
        this.minDamage = 24; // stronger
        this.maxDamage = 36;
    ...
    public static void load() {
        if (texture == null) {
            texture = new Texture(Gdx.files.internal("bullet3.png"));
            region = new TextureRegion(texture);
        }
        if (webTexture == null) {
            webTexture = new Texture(Gdx.files.internal("web3.png"));
