...
public class Bullet2 extends Bullet {
    ...
    public Bullet2(float x, float y, float angleDeg) {
        this(x, y, angleDeg, DEFAULT_SPEED);
    }

    public Bullet2(float x, float y, float angleDeg, float speed) {
        super(x, y, angleDeg, speed, region, webRegion);
        this.minDamage = 10; // stronger
        this.maxDamage = 18;
        this.cost = 12; // costs more points to fire
    ...
    public static void load() {
        if (texture == null) {
            texture = new Texture(Gdx.files.internal("bullet2.png"));
            region = new TextureRegion(texture);
        }
        if (webTexture == null) {
            webTexture = new Texture(Gdx.files.internal("web2.png"));
