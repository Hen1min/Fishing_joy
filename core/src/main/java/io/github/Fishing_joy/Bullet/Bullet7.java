...
public class Bullet7 extends Bullet {
    ...
    public Bullet7(float x, float y, float angleDeg) {
        this(x, y, angleDeg, DEFAULT_SPEED);
    }

    public Bullet7(float x, float y, float angleDeg, float speed) {
        super(x, y, angleDeg, speed, region, webRegion);
        this.minDamage = 120; // stronger
        this.maxDamage = 210;
        this.cost = 95; // costs more points to fire
    ...
    public static void load() {
        if (texture == null) {
            texture = new Texture(Gdx.files.internal("bullet7.png"));
            region = new TextureRegion(texture);
        }
        if (webTexture == null) {
            webTexture = new Texture(Gdx.files.internal("web7.png"));
